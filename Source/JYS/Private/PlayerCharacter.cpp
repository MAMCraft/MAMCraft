...
APlayerCharacter::APlayerCharacter()
{
	...
	// sound
	static ConstructorHelpers::FObjectFinder<USoundWave> onearrowAsset(TEXT("/Script/Engine.SoundWave'/Game/KYH/Sound/bow_one.bow_one'"));
	if (onearrowAsset.Succeeded())
		onearrow = onearrowAsset.Object;
	static ConstructorHelpers::FObjectFinder<USoundWave> threearrowAsset(TEXT("/Script/Engine.SoundWave'/Game/KYH/Sound/bow_three.bow_three'"));
	if (threearrowAsset.Succeeded())
		threearrow = threearrowAsset.Object;
	static ConstructorHelpers::FObjectFinder<USoundWave> twoswordAsset(TEXT("/Script/Engine.SoundWave'/Game/KYH/Sound/two_sword.two_sword'"));
	if (twoswordAsset.Succeeded())
		twosword = twoswordAsset.Object;
	static ConstructorHelpers::FObjectFinder<USoundWave> threeswordAsset(TEXT("/Script/Engine.SoundWave'/Game/KYH/Sound/three_sword.three_sword'"));
	if (threeswordAsset.Succeeded())
		threesword = threeswordAsset.Object;
	...
	// 데미지 받으면 빨간색으로 깜빡
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> RedMaterialFinder(TEXT("/Script/Engine.Material'/Game/GameResource/Player/Mesh/MI_DamageRed.MI_DamageRed'"));
...
void APlayerCharacter::UpgradeSkill()
{
	...
	if (bowState == 0)
	{
		...
		//sound
		UGameplayStatics::PlaySound2D(this, onearrow, 2.f);
	}
	else if (bowState == 1)
	{
		//UE_LOG(LogTemp, Log, TEXT("%d UpgradeSkill"),bowState);
		...
		UGameplayStatics::PlaySound2D(this, threearrow, 2.f);
...
void APlayerCharacter::comboAttack()
{
	...
	else
	{
		...
		{
			//sound
			UGameplayStatics::PlaySound2D(this, twosword, 2.f);
		}
		AttackStartComboState();
